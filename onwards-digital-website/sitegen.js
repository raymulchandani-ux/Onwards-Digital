...
window.SITEGEN = (function () {
  ...
  var PRESETS = {
    restaurant: {
      ...
      img: { hero: U("1675583690138-5242aee4c0c3"), x1: U("1642501500606-b56852b10732"), x2: U("1675583635076-6eac784dbf41"), about: U("1569767143908-1342240550ce"), g: [U("1650223164911-1274a3d15700", 900), U("1727005217349-66276af3aa7d", 900), U("1756925974940-fbf149fa0fd2", 900), U("1642501500606-b56852b10732", 900), U("1569767143908-1342240550ce", 900), U("1675583635076-6eac784dbf41", 900)] },
  ...
  function defaults(key) {
    var s = {
      preset: "restaurant", name: "", tagline: "", onePage: false, credit: true,
  ...
  function heroActs(c) {
    var h = c.s.hero, out = "";
    if (h.cta && c.target) out += '<a class="btn" href="#' + c.target + '">' + esc(h.cta) + "</a>";
    if (h.cta2 && c.has("services")) out += '<a class="btn alt" href="#services">' + esc(h.cta2) + "</a>";
    return out ? '<div class="acts">' + out + "</div>" : "";
  }
  function heroText(c, noKick) {
    var h = c.s.hero;
    return (h.kick && !noKick ? '<p class="kick">' + esc(h.kick) + "</p>" : "") + "<h1>" + esc(h.title) + "</h1>" + (h.text ? '<p class="lede">' + esc(h.text) + "</p>" : "") + heroActs(c);
  }
  function sh(title, intro, center) { return '<div class="sh' + (center ? " c" : "") + '"><h2>' + esc(title) + "</h2>" + (intro ? '<p class="lede">' + esc(intro) + "</p>" : "") + "</div>"; }
  function facts(c) { var f = c.s.about.facts || []; return '<div class="facts">' + f.map(function (x) { return "<div><b>" + esc(x[0]) + "</b><span>" + esc(x[1]) + "</span></div>"; }).join("") + "</div>"; }

  /* ── Header designs ────────────────────────────────────── */
  function logo(c) { return '<a class="logo" href="#home">' + esc(c.s.name) + "</a>"; }
  ...
  var HEADERS = [
    { n: "Classic", css: ".hd-1 .links{margin-left:auto}", html: function (c) { return '<div class="w bar">' + logo(c) + links(c) + hEnd(c) + "</div>"; } },
    { n: "Centred", css: ".hd-2 .bar{flex-direction:column;gap:14px;padding:22px 0 14px}.hd-2 .logo{font-size:2.1em}.hd-2 .cta{display:none}.hd-2 .end{position:absolute;right:var(--gut);top:18px}@media(max-width:760px){.hd-2 .bar{flex-direction:row;padding:0}.hd-2 .logo{font-size:1.35em}.hd-2 .end{position:static}}", html: function (c) { return '<div class="w bar">' + logo(c) + links(c) + hEnd(c) + "</div>"; } },
    { n: "Split", css: ".hd-3 .bar{display:grid;grid-template-columns:1fr auto 1fr}.hd-3 .logo{grid-column:2;grid-row:1;text-align:center}.hd-3 .links{grid-column:1;grid-row:1}.hd-3 .end{grid-column:3;grid-row:1;justify-self:end}@media(max-width:760px){.hd-3 .bar{grid-template-columns:1fr auto}.hd-3 .logo{grid-column:1;text-align:left}.hd-3 .end{grid-column:2}}", html: function (c) { return '<div class="w bar">' + logo(c) + links(c) + hEnd(c) + "</div>"; } },
    { n: "Floating", css: ".hd-4.s{background:transparent;border:0;padding-top:14px}.hd-4 .bar{background:var(--sbg);border:1px solid var(--sline);border-radius:calc(var(--r) + 4px);padding:0 10px 0 24px;min-height:64px;box-shadow:0 14px 34px -24px rgba(0,0,0,.45)}.hd-4 .links{margin-left:auto}.hd-4{margin-bottom:-78px}@media(max-width:760px){.hd-4{padding-top:10px;margin-bottom:-72px}.hd-4 .bar{padding-left:16px;min-height:56px}}", html: function (c) { return '<div class="w bar">' + logo(c) + links(c) + hEnd(c) + "</div>"; } },
    { n: "Minimal", css: ".hd-5 .links{display:none;position:absolute;right:var(--gut);left:auto;top:100%;min-width:230px;background:var(--sbg);flex-direction:column;align-items:flex-start;padding:18px 22px 22px;gap:14px;border:1px solid var(--sline);border-top:0}.hd-5.open .links{display:flex}.hd-5 .menu-btn{display:flex}.hd-5 .end{margin-left:auto}", html: function (c) { return '<div class="w bar">' + logo(c) + links(c) + hEnd(c) + "</div>"; } },
    { n: "Info strip", css: ".hd-6 .strip{background:var(--bb);color:var(--bt);font-size:.8em}.hd-6 .strip .w{display:flex;justify-content:space-between;gap:16px;min-height:34px;align-items:center}.hd-6 .links{margin-left:auto}@media(max-width:760px){.hd-6 .strip span+span{display:none}}", html: function (c) { var h = lines(c.s.contact.hours)[0] || ""; return '<div class="strip"><div class="w"><span>' + esc(h) + "</span><span>" + esc(c.s.contact.phone) + '</span></div></div><div class="w bar">' + logo(c) + links(c) + hEnd(c) + "</div>"; } }
  ];
  ...
  var HEROES = [
    ...
    { n: "Editorial", css: ".he-3 .w{padding-block:clamp(40px,7vw,96px)}.he-3 h1{font-size:calc(clamp(44px,8vw,128px)*var(--k,1)*var(--hk));max-width:15ch}.he-3 .row{display:flex;justify-content:space-between;align-items:flex-end;gap:32px;margin:30px 0 36px;padding-top:24px;border-top:1px solid var(--sline)}.he-3 .row .lede,.he-3 .row .acts{margin-top:0}.he-3 .wide{aspect-ratio:21/9}@media(max-width:760px){.he-3 .row{flex-direction:column;align-items:flex-start;gap:18px}.he-3 .wide{aspect-ratio:4/3}}",
      html: function (c) { var h = c.s.hero; return '<div class="w">' + (h.kick ? '<p class="kick">' + esc(h.kick) + "</p>" : "") + "<h1>" + esc(h.title) + '</h1><div class="row">' + (h.text ? '<p class="lede">' + esc(h.text) + "</p>" : "<span></span>") + heroActs(c) + '</div><div class="ph wide">' + im(c, "hero") + "</div></div>"; } },
    ...
    { n: "Details line", css: ".he-9 .top{display:grid;grid-template-columns:1.4fr 1fr;gap:48px;align-items:end;padding-top:clamp(48px,7vw,100px)}.he-9 .top .lede{margin-top:0}.he-9 .dl{display:grid;grid-template-columns:repeat(3,1fr);border-top:1px solid var(--sline);border-bottom:1px solid var(--sline);margin:clamp(32px,4vw,56px) 0}.he-9 .dl div{padding:16px 18px;display:flex;flex-direction:column;gap:2px;font-size:.92em}.he-9 .dl div:first-child{padding-left:0}.he-9 .dl div+div{border-left:1px solid var(--sline)}.he-9 .pano{aspect-ratio:21/8;margin-bottom:clamp(48px,7vw,96px)}@media(max-width:760px){.he-9 .top{grid-template-columns:1fr;gap:8px}.he-9 .dl div{padding:12px 10px;font-size:.78em}.he-9 .pano{aspect-ratio:16/10}}",
      html: function (c) { var ct = c.s.contact; return '<div class="w"><div class="top"><div>' + (c.s.hero.kick ? '<p class="kick">' + esc(c.s.hero.kick) + "</p>" : "") + "<h1>" + esc(c.s.hero.title) + "</h1></div><div>" + (c.s.hero.text ? '<p class="lede">' + esc(c.s.hero.text) + "</p>" : "") + heroActs(c) + '</div></div><div class="dl"><div><span class="mut">Open</span><span>' + esc(lines(ct.hours)[0] || "") + '</span></div><div><span class="mut">Find us</span><span>' + esc(lines(ct.address)[0] || "") + '</span></div><div><span class="mut">Call</span><span>' + esc(ct.phone) + '</span></div></div><div class="ph pano">' + im(c, "hero") + "</div></div>"; } },
  ...
  function highlights(c) {
    ...
    return '<section class="s t-' + (c.s.hero.tone === "ink" || c.s.hero.tone === "accent" ? c.s.hero.tone : "base") + ' hl"><div class="w g">' + c.s.hl.map(function (x) { return "<div><h3>" + esc(x[0]) + '</h3><p class="mut">' + esc(x[1]) + "</p></div>"; }).join("") + "</div></section>";
  }

  /* ── About designs (text boxes with images) ────────────── */
  function aboutText(c, noFacts) { var a = c.s.about; return "<h2>" + esc(a.title) + '</h2><div class="body" style="margin-top:22px">' + paras(a.text) + "</div>" + (noFacts ? "" : facts(c)); }
  var ABOUTS = [
    ...
    { n: "Statement", css: ".ab-5 h2.big{font-size:calc(clamp(34px,5.2vw,76px)*var(--k,1)*var(--hk));max-width:18ch}.ab-5 .g{display:grid;grid-template-columns:1fr 1.6fr;gap:clamp(24px,5vw,72px);margin-top:clamp(32px,5vw,64px);padding-top:28px;border-top:1px solid var(--sline);align-items:start}.ab-5 .ph{aspect-ratio:4/3}@media(max-width:760px){.ab-5 .g{grid-template-columns:1fr 1.3fr;gap:16px}.ab-5 .body{font-size:.92em}}",
      html: function (c) { return '<div class="w pad"><h2 class="big">' + esc(c.s.about.title) + '</h2><div class="g"><div class="ph">' + im(c, "about") + '</div><div class="body">' + paras(c.s.about.text) + "</div></div></div>"; } },
    { n: "Wide photo", css: ".ab-6 .wide{aspect-ratio:21/8}.ab-6 .g{display:grid;grid-template-columns:1fr 1.3fr;gap:clamp(24px,5vw,72px);margin-top:clamp(32px,5vw,56px)}.ab-6 .facts{margin-top:28px}@media(max-width:760px){.ab-6 .wide{aspect-ratio:16/10}.ab-6 .g{grid-template-columns:1fr;gap:16px}}",
      html: function (c) { return '<div class="w pad"><div class="ph wide">' + im(c, "about") + '</div><div class="g"><h2>' + esc(c.s.about.title) + '</h2><div><div class="body">' + paras(c.s.about.text) + "</div>" + facts(c) + "</div></div></div>"; } },
    ...
    { n: "Big numbers", css: ".ab-8 .g{display:grid;grid-template-columns:1fr 1.2fr;gap:clamp(24px,5vw,72px)}.ab-8 .facts{margin-top:clamp(36px,5vw,64px)}.ab-8 .facts b{font-size:calc(clamp(40px,5.4vw,80px)*var(--hk))}.ab-8 .strip{display:grid;grid-template-columns:2fr 1fr;gap:clamp(8px,1.4vw,16px);margin-top:clamp(36px,5vw,56px)}.ab-8 .strip .ph{aspect-ratio:16/8}.ab-8 .strip .ph+.ph{aspect-ratio:auto}@media(max-width:760px){.ab-8 .g{grid-template-columns:1fr;gap:12px}.ab-8 .facts b{font-size:2em}}",
      html: function (c) { return '<div class="w pad"><div class="g"><h2>' + esc(c.s.about.title) + '</h2><div class="body">' + paras(c.s.about.text) + "</div></div>" + facts(c) + '<div class="strip"><div class="ph">' + im(c, "about") + '</div><div class="ph">' + im(c, "x1") + "</div></div></div>"; } },
  ...
  function items(c) { return (c.s.services.items || []).filter(function (x) { return x.n; }); }
  function price(p) { return p ? esc(p) : ""; }
  var SERVICES = [
    { n: "Price list", css: ".sv-1 .list{display:grid;grid-template-columns:1fr 1fr;column-gap:clamp(32px,6vw,96px);row-gap:26px}.sv-1 .top{display:flex;align-items:baseline;gap:10px}.sv-1 .top i{flex:1;border-bottom:1px dotted var(--smut);transform:translateY(-5px)}.sv-1 .n{font-family:var(--hf),serif;font-weight:var(--hw);font-size:1.22em}.sv-1 .d{font-size:.92em;margin-top:4px}@media(max-width:760px){.sv-1 .list{grid-template-columns:1fr;row-gap:18px}}",
      html: function (c) { return '<div class="w pad">' + sh(c.s.services.title, c.s.services.intro) + '<div class="list">' + items(c).map(function (x) { return '<div><div class="top"><span class="n">' + esc(x.n) + "</span><i></i><span>" + price(x.p) + '</span></div><p class="mut d">' + esc(x.d) + "</p></div>"; }).join("") + "</div></div>"; } },
    { n: "Cards", css: ".sv-2 .grid{display:grid;grid-template-columns:repeat(3,1fr);gap:clamp(10px,1.6vw,20px)}.sv-2 .card{border:1px solid var(--sline);border-radius:var(--r);padding:clamp(18px,2.4vw,30px);display:flex;flex-direction:column;gap:10px;min-height:190px}.sv-2 .card p{font-size:.92em}.sv-2 .p{margin-top:auto;font-family:var(--hf),serif;font-size:1.5em}@media(max-width:760px){.sv-2 .grid{grid-template-columns:1fr 1fr}.sv-2 .card{min-height:0;padding:14px;font-size:.88em}}",
      html: function (c) { return '<div class="w pad">' + sh(c.s.services.title, c.s.services.intro) + '<div class="grid">' + items(c).map(function (x) { return '<div class="card"><h3>' + esc(x.n) + '</h3><p class="mut">' + esc(x.d) + '</p><span class="p">' + price(x.p) + "</span></div>"; }).join("") + "</div></div>"; } },
    { n: "Rows", css: ".sv-3 .row{display:grid;grid-template-columns:1.1fr 2fr auto;gap:24px;padding:20px 0;border-top:1px solid var(--sline);align-items:baseline}.sv-3 .row:last-child{border-bottom:1px solid var(--sline)}.sv-3 .n{font-family:var(--hf),serif;font-weight:var(--hw);font-size:1.25em}@media(max-width:760px){.sv-3 .row{grid-template-columns:1fr auto;gap:4px 16px;padding:14px 0}.sv-3 .row .mut{grid-column:1/-1;grid-row:2;font-size:.9em}}",
      html: function (c) { return '<div class="w pad">' + sh(c.s.services.title, c.s.services.intro) + items(c).map(function (x) { return '<div class="row"><span class="n">' + esc(x.n) + '</span><span class="mut">' + esc(x.d) + "</span><span>" + price(x.p) + "</span></div>"; }).join("") + "</div>"; } },
    { n: "Menu card", css: ".sv-4 .card{max-width:680px;margin-inline:auto;text-align:center;border:1px solid var(--sline);border-radius:var(--r);padding:clamp(32px,6vw,72px) clamp(20px,5vw,64px)}.sv-4 .it{padding:16px 0}.sv-4 .it+.it{border-top:1px solid var(--sline)}.sv-4 .n{font-family:var(--hf),serif;font-weight:var(--hw);font-size:1.3em}.sv-4 .d{font-style:italic;font-size:.94em}.sv-4 .p{display:block;margin-top:4px;font-size:.92em}",
      html: function (c) { return '<div class="w pad"><div class="card">' + sh(c.s.services.title, c.s.services.intro, true) + items(c).map(function (x) { return '<div class="it"><div class="n">' + esc(x.n) + '</div><div class="mut d">' + esc(x.d) + '</div><span class="p">' + price(x.p) + "</span></div>"; }).join("") + "</div></div>"; } },
    { n: "Side heading", css: ".sv-5 .g{display:grid;grid-template-columns:1fr 1.5fr;gap:clamp(28px,6vw,96px);align-items:start}.sv-5 .sh{position:sticky;top:100px}.sv-5 .row{display:flex;justify-content:space-between;gap:20px;padding:18px 0;border-bottom:1px solid var(--sline)}.sv-5 .row:first-child{border-top:1px solid var(--sline)}.sv-5 .n{font-family:var(--hf),serif;font-weight:var(--hw);font-size:1.2em;display:block}.sv-5 .row .mut{font-size:.9em}@media(max-width:760px){.sv-5 .g{grid-template-columns:1fr;gap:0}.sv-5 .sh{position:static}}",
      html: function (c) { return '<div class="w g pad">' + sh(c.s.services.title, c.s.services.intro) + "<div>" + items(c).map(function (x) { return '<div class="row"><div><span class="n">' + esc(x.n) + '</span><span class="mut">' + esc(x.d) + "</span></div><span>" + price(x.p) + "</span></div>"; }).join("") + "</div></div>"; } },
    { n: "With photo", css: ".sv-6 .g{display:grid;grid-template-columns:1fr 1.1fr;gap:clamp(28px,6vw,96px);align-items:start}.sv-6 .ph{aspect-ratio:4/5;position:sticky;top:100px}.sv-6 .row{display:grid;grid-template-columns:1fr auto;gap:2px 20px;padding:16px 0;border-top:1px solid var(--sline)}.sv-6 .n{font-family:var(--hf),serif;font-weight:var(--hw);font-size:1.18em}.sv-6 .row .mut{grid-column:1/-1;font-size:.9em}@media(max-width:760px){.sv-6 .g{grid-template-columns:1fr;gap:28px}.sv-6 .ph{position:static;aspect-ratio:16/9}}",
      html: function (c) { return '<div class="w g pad"><div class="ph">' + im(c, "x1") + "</div><div>" + sh(c.s.services.title, c.s.services.intro) + items(c).map(function (x) { return '<div class="row"><span class="n">' + esc(x.n) + "</span><span>" + price(x.p) + '</span><span class="mut">' + esc(x.d) + "</span></div>"; }).join("") + "</div></div>"; } }
  ];

  /* ── Gallery designs ───────────────────────────────────── */
  function gimgs(c, n) { var out = []; for (var i = 0; i < (n || 6); i++) out.push('<div class="ph">' + im(c, "g" + i) + "</div>"); return out.join(""); }
  var GALLERIES = [
    { n: "Grid", css: ".ga-1 .grid{display:grid;grid-template-columns:repeat(3,1fr);gap:clamp(6px,1.2vw,16px)}.ga-1 .ph{aspect-ratio:1}", html: function (c) { return '<div class="w pad">' + sh(c.s.gallery.title) + '<div class="grid">' + gimgs(c) + "</div></div>"; } },
    { n: "Feature", css: ".ga-2 .grid{display:grid;grid-template-columns:repeat(4,1fr);grid-auto-rows:clamp(100px,16vw,230px);gap:clamp(6px,1.2vw,16px)}.ga-2 .ph:first-child{grid-column:span 2;grid-row:span 2}", html: function (c) { return '<div class="w pad">' + sh(c.s.gallery.title) + '<div class="grid">' + gimgs(c, 5) + "</div></div>"; } },
    { n: "Strip", css: ".ga-3 .strip{display:grid;grid-auto-flow:column;grid-auto-columns:clamp(220px,28vw,380px);gap:16px;overflow-x:auto;scroll-snap-type:x mandatory;padding-bottom:10px;scrollbar-width:thin}.ga-3 .ph{aspect-ratio:4/5;scroll-snap-align:start}@media(max-width:760px){.ga-3 .strip{grid-auto-columns:62%;gap:10px}}", html: function (c) { return '<div class="w pad">' + sh(c.s.gallery.title) + '<div class="strip">' + gimgs(c) + "</div></div>"; } },
    { n: "Mosaic", css: ".ga-4 .cols{columns:3;column-gap:clamp(8px,1.4vw,16px)}.ga-4 .ph{margin-bottom:clamp(8px,1.4vw,16px);break-inside:avoid;aspect-ratio:4/5}.ga-4 .ph:nth-child(3n+2){aspect-ratio:1}.ga-4 .ph:nth-child(3n){aspect-ratio:3/4}", html: function (c) { return '<div class="w pad">' + sh(c.s.gallery.title) + '<div class="cols">' + gimgs(c) + "</div></div>"; } },
  ...
  function pSubmit(c) { return '<p class="err-msg" aria-live="polite"></p><div><button class="btn" type="submit">' + esc(c.s.booking.button || "Send request") + '</button></div><div class="sent-msg"><h3>Request sent.</h3><p class="mut">Thank you. We\'ll confirm by email shortly.</p></div>'; }
  ...
  var BOOKINGS = [
    { n: "Centred card", css: ".bk-1 .card{max-width:760px;margin-inline:auto;border:1px solid var(--sline);border-radius:var(--r);padding:clamp(24px,5vw,56px)}", html: function (c) { return '<div class="w pad"><div class="card">' + sh(c.s.booking.title, c.s.booking.intro, true) + fullForm(c) + "</div></div>"; } },
    { n: "Photo split", css: ".bk-2 .g{display:grid;grid-template-columns:1fr 1.1fr;gap:clamp(28px,5vw,80px);align-items:stretch}.bk-2 .ph{min-height:100%;aspect-ratio:auto}@media(max-width:760px){.bk-2 .g{grid-template-columns:1fr}.bk-2 .ph{aspect-ratio:16/9;min-height:0}}", html: function (c) { return '<div class="w g pad"><div class="ph">' + im(c, "x1") + "</div><div>" + sh(c.s.booking.title, c.s.booking.intro) + fullForm(c) + "</div></div>"; } },
    { n: "Form & details", css: ".bk-3 .g{display:grid;grid-template-columns:1.5fr 1fr;gap:clamp(28px,6vw,96px);align-items:start}.bk-3 .info{display:grid;gap:22px;padding-top:24px;border-top:1px solid var(--sline);position:sticky;top:100px}.bk-3 .info p{font-size:.95em}@media(max-width:760px){.bk-3 .g{grid-template-columns:1fr}.bk-3 .info{position:static;grid-template-columns:1fr 1fr}.bk-3 .info>div:first-child{grid-column:1/-1}}", html: function (c) { return '<div class="w g pad"><div>' + sh(c.s.booking.title, c.s.booking.intro) + fullForm(c) + "</div>" + infoBlock(c) + "</div>"; } },
    { n: "Band", css: ".bk-4 .g{display:grid;grid-template-columns:1fr 1.4fr;gap:clamp(28px,6vw,96px);align-items:start}@media(max-width:760px){.bk-4 .g{grid-template-columns:1fr;gap:0}}", html: function (c) { return '<div class="w g pad">' + sh(c.s.booking.title, c.s.booking.intro) + fullForm(c) + "</div>"; } },
    ...
    { n: "Choose first", css: ".bk-9 .g{display:grid;grid-template-columns:1fr 1.3fr;gap:clamp(28px,5vw,80px);align-items:start}.bk-9 .chips.big{display:grid;gap:8px}.bk-9 .chips.big .chip{text-align:left;padding:1.1em 1.2em;font-size:1em;font-family:var(--hf),serif;font-weight:var(--hw)}@media(max-width:760px){.bk-9 .g{grid-template-columns:1fr;gap:18px}.bk-9 .chips.big{grid-template-columns:1fr 1fr}}",
      html: function (c) { return '<div class="w pad">' + sh(c.s.booking.title, c.s.booking.intro) + formOpen(c) + '<div class="g"><div>' + pOpts(c, true) + '</div><div class="bk-form">' + pickers(c, { noOpts: true }) + pDetails() + pSubmit(c) + "</div></div></form></div>"; } },
    { n: "Call or book", css: ".bk-10 .g{display:grid;grid-template-columns:1fr 1.3fr;gap:clamp(28px,6vw,96px);align-items:start}.bk-10 .call{display:grid;gap:10px;margin-top:28px;padding-top:24px;border-top:1px solid var(--sline)}.bk-10 .call a{font-family:var(--hf),serif;font-weight:var(--hw);font-size:calc(clamp(26px,3.2vw,44px)*var(--hk));text-decoration:none;line-height:1.1}@media(max-width:760px){.bk-10 .g{grid-template-columns:1fr;gap:28px}}",
      html: function (c) { var ct = c.s.contact; return '<div class="w g pad"><div>' + sh(c.s.booking.title, c.s.booking.intro) + '<div class="call"><span class="lb">Prefer to call?</span><a href="tel:' + esc(String(ct.phone).replace(/[^\d+]/g, "")) + '">' + esc(ct.phone) + '</a><a href="mailto:' + esc(ct.email) + '" style="font-size:1.1em;font-family:inherit">' + esc(ct.email) + "</a></div></div>" + fullForm(c) + "</div>"; } }
  ];
  ...
  function tel(c) { return '<a href="tel:' + esc(String(c.s.contact.phone).replace(/[^\d+]/g, "")) + '">' + esc(c.s.contact.phone) + "</a>"; }
  function mail(c) { return '<a href="mailto:' + esc(c.s.contact.email) + '">' + esc(c.s.contact.email) + "</a>"; }
  ...
  var CONTACTS = [
    { n: "Columns", css: ".ct-1 .cols{display:grid;grid-template-columns:repeat(3,1fr);gap:clamp(16px,3vw,40px)}.ct-1 .cols>div{border-top:1px solid var(--sline);padding-top:16px;display:grid;gap:6px;align-content:start}.ct-1 .note{margin-top:32px}@media(max-width:760px){.ct-1 .cols{grid-template-columns:1fr 1fr}.ct-1 .cols>div:last-child{grid-column:1/-1}}",
      html: function (c) { var ct = c.s.contact; return '<div class="w pad">' + sh(ct.title) + '<div class="cols"><div><span class="lb">Visit</span><p>' + addr(c) + '</p></div><div><span class="lb">Hours</span>' + lines(ct.hours).map(function (l) { return "<p>" + esc(l) + "</p>"; }).join("") + '</div><div><span class="lb">Contact</span><p>' + tel(c) + "</p><p>" + mail(c) + "</p></div></div>" + (ct.note ? '<p class="mut note">' + esc(ct.note) + "</p>" : "") + "</div>"; } },
    ...
    { n: "Big type", css: ".ct-4 .c{text-align:center}.ct-4 .big{display:grid;gap:4px;margin:clamp(24px,4vw,48px) 0}.ct-4 .big a{font-family:var(--hf),serif;font-weight:var(--hw);font-size:calc(clamp(30px,4.6vw,68px)*var(--hk));line-height:1.15;word-break:break-word}.ct-4 .two{display:grid;grid-template-columns:1fr 1fr;gap:24px;max-width:640px;margin-inline:auto;padding-top:24px;border-top:1px solid var(--sline);text-align:left}",
      html: function (c) { var ct = c.s.contact; return '<div class="w pad c"><h2>' + esc(ct.title) + '</h2><div class="big">' + tel(c) + mail(c) + '</div><div class="two"><div><span class="lb">Address</span><p>' + addr(c) + '</p></div><div><span class="lb">Hours</span>' + lines(ct.hours).map(function (l) { return "<p>" + esc(l) + "</p>"; }).join("") + "</div></div></div>"; } },
    { n: "Hours table", css: ".ct-5 .g{display:grid;grid-template-columns:1fr 1.1fr;gap:clamp(28px,6vw,96px)}.ct-5 .d{display:grid;gap:18px;margin-top:8px}.ct-5 .t{border-top:1px solid var(--sline)}@media(max-width:760px){.ct-5 .g{grid-template-columns:1fr;gap:24px}.ct-5 .d{grid-template-columns:1fr 1fr}}",
      html: function (c) { var ct = c.s.contact; return '<div class="w g pad"><div>' + sh(ct.title, ct.note) + '<div class="d"><div><span class="lb">Address</span><p>' + addr(c) + '</p></div><div><span class="lb">Contact</span><p>' + tel(c) + "</p><p>" + mail(c) + '</p></div></div></div><div><span class="lb">Opening hours</span><div class="t" style="margin-top:10px">' + hoursRows(c) + "</div></div></div>"; } },
  ...
  var FOOTERS = [
    { n: "Columns", css: ".ft-1 .g{display:grid;grid-template-columns:1.4fr 1fr 1fr;gap:32px;padding-block:clamp(40px,6vw,72px) 32px}.ft-1 .g>div{display:grid;gap:8px;align-content:start;font-size:.94em}.ft-1 .logo{font-size:1.8em;margin-bottom:6px}.ft-1 .small{padding:20px 0 28px;border-top:1px solid var(--sline)}@media(max-width:760px){.ft-1 .g{grid-template-columns:1fr 1fr}.ft-1 .g>div:first-child{grid-column:1/-1}}",
      html: function (c) { return '<div class="w g"><div>' + logo(c) + '<p class="mut">' + esc(c.s.tagline) + "</p></div><div>" + flinks(c) + '</div><div><p>' + addr(c) + "</p><p>" + tel(c) + "</p><p>" + mail(c) + '</p></div></div><div class="w small"><span>&copy; ' + new Date().getFullYear() + " " + esc(c.s.name) + "</span>" + credit(c) + "</div>"; } },
    { n: "Centred", css: ".ft-2 .in{text-align:center;display:grid;gap:22px;justify-items:center;padding-block:clamp(48px,7vw,88px) 28px}.ft-2 .logo{font-size:2.4em}.ft-2 nav{display:flex;flex-wrap:wrap;gap:10px 28px;justify-content:center}.ft-2 .small{justify-content:center;padding-bottom:28px}",
      html: function (c) { return '<div class="w in">' + logo(c) + '<p class="mut">' + esc(c.s.tagline) + "</p><nav>" + flinks(c) + '</nav></div><div class="w small"><span>&copy; ' + new Date().getFullYear() + " " + esc(c.s.name) + " &middot; " + esc(c.s.contact.phone) + "</span>" + credit(c) + "</div>"; } },
    { n: "One line", css: ".ft-3 .in{display:flex;flex-wrap:wrap;gap:12px 28px;align-items:center;justify-content:space-between;padding-block:28px;font-size:.9em;border-top:1px solid var(--sline)}.ft-3 nav{display:flex;flex-wrap:wrap;gap:8px 22px}",
      html: function (c) { return '<div class="w in"><span>&copy; ' + new Date().getFullYear() + " " + esc(c.s.name) + "</span><nav>" + flinks(c) + "</nav>" + (credit(c) || "<span></span>") + "</div>"; } },
    { n: "Big name", css: ".ft-4{overflow:hidden}.ft-4 .top{display:flex;flex-wrap:wrap;justify-content:space-between;gap:16px 40px;padding-top:clamp(40px,6vw,72px);font-size:.94em}.ft-4 nav{display:flex;flex-wrap:wrap;gap:8px 24px}.ft-4 .huge{font-family:var(--hf),serif;font-weight:var(--hw);font-size:clamp(56px,13vw,210px);line-height:.9;letter-spacing:-.03em;white-space:nowrap;margin:clamp(28px,5vw,64px) 0 -0.12em;padding-inline:var(--gut)}.ft-4 .small{padding-bottom:22px;padding-top:28px}",
      html: function (c) { return '<div class="w top"><nav>' + flinks(c) + "</nav><span>" + esc(lines(c.s.contact.address).join(", ")) + " &middot; " + tel(c) + '</span></div><div class="huge">' + esc(c.s.name) + '</div><div class="w small"><span>&copy; ' + new Date().getFullYear() + " " + esc(c.s.name) + "</span>" + credit(c) + "</div>"; } }
  ];
  ...
  function css(s, only) {
    ...
    var keys = only ? (only === "hero" ? ["header", "hero"] : [only]) : ["header", "hero", "about", "services", "gallery", "booking", "contact", "footer"];
    keys.forEach(function (k) { parts.push(pick(k, s).t.css); });
    if (!only || only === "hero") parts.push(HL_CSS);
    return parts.join("\n");
  }

  function bodyHTML(s, only) {
    var c = ctx(s);
    if (only) {
      if (only === "header") return renderHeader(c) + '<div class="s t-' + s.hero.tone + '" style="height:900px"></div>';
  ...
  var RUNTIME = "(" + function () {
    ...
    function show(id, scroll) {
      ...
      if (scroll !== false) {
        ...
        else { var t = d.getElementById("pg-" + id); if (t) { if (id === "home") window.scrollTo({ top: 0, behavior: "smooth" }); else t.scrollIntoView({ behavior: "smooth" }); } }
      }
    ...
    function fill() {
      ...
      [].forEach.call(d.querySelectorAll("[data-cal]"), function (el) { var n = new Date(); buildCal(el, n.getFullYear(), n.getMonth()); });
    }
  ...
  function doc(s, o) {
    ...
    var head = '<meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1">' +
      ...
      (o.preview ? "<style>html{scrollbar-width:thin}</style>" : "");
    return "<!DOCTYPE html><html lang=\"en\"" + (o.preview || o.thumb ? " data-preview" : "") + "><head>" + head + '</head><body class="' + bodyClass(s) + '"><div id="root">' + bodyHTML(s, o.only) + "</div>" + (o.thumb ? "" : "<script>" + RUNTIME + "<\/script>") + "</body></html>";
  }
