...
- **"Our work, up close" is gone on every size (Sept 16).** "What we build" (`#build`, `.grid-5.cases`) now carries each demo's full write-up — category · city cap, serif name, lede, three-point list, "Open the site". Three across on laptop, two on tablet (≤1100), one on phone (lede clamped to 3 lines, list and link hidden). Photos are 4:3 with 14 px corners (12 px on phone), a faint inset hairline and a small lift + zoom on hover — no boxed hover border. The live iframe previews and the `.frame` fitter in `site.js` were removed. Nav has no "Our work" link; hero "See our work" and thanks.html point to `#build`.
